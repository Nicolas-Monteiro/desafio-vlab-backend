...
MEDIAS_PADRAO = {"GASOLINA": 5.50, "ETANOL": 3.80, "DIESEL": 6.00}
...
class AbastecimentoService:
    ...
    async def listar_com_filtros(
        ...
        cache_key = (
            f"abastecimentos:page:{page}:size:{size}:"
            f"tipo:{tipo_combustivel}:data:{data_filtro}"
        )
    ...
    async def buscar_por_motorista(db: AsyncSession, cpf: str) -> list[dict]:
        ...
        query = (
            ...
            .where(AbastecimentoModel.cpf_motorista == cpf_limpo)
            .order_by(AbastecimentoModel.data_hora.desc())
        )
        result = await db.execute(query)
        abastecimentos = result.scalars().all()
